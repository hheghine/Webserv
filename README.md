...
```
        ...
        :+:      :+:    :+:       The web server can handle HTTP GET, HEAD, POST, PUT, and DELETE Requests, and can
      +:+ +:+         +:+         serve static files from a specified root directory or dynamic content using CGI.
    +#+  +:+       +#+            It is also able to handle multiple client connections concurrently with the help
  +#+#+#+#+#+   +#+               of select().
       #+#    #+#            
      ###   ########.fr           June 2024
```
